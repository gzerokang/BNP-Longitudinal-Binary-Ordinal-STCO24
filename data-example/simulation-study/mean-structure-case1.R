...
cov.mtx.true=power_exp(D.mtx)
noise1=rmvnorm(n,rep(0,T.all),cov.mtx.true)
Z.true1=t(apply(noise1, MARGIN=1, function(vec){vec+mean.vec.true})) ## continuous signal
## adding the measurement error
set.seed(1)
measure.error=matrix(rnorm(n*length(grid.T[T.idx]),mean=0,sd=sqrt(0.1)),nrow=n)
latent.process.mtx=Z.true1+measure.error
true.prob.curve.mtx1=expit(latent.process.mtx)
...
system.time(
  for (i in 1:nsim) {
    ...
    ## sample mu0
    mu0.cur=sample_mu0(mu.cur,Sigma.cur.inv,nu.cur,a.mu,b.mu)
...
RMSE.signal.full.GP.75=apply(post.Z.sample1, MARGIN=2, function(vec){
...
## RMSE of probability
RMSE.prob.full.GP.75=apply(post.prob.sample1, MARGIN=2, function(vec){
  mtx.cur=matrix(vec,nrow=n,byrow=TRUE)
  ...
  for (id.sub in 1:n) {
    ...
    true.prob.use=true.prob.curve.mtx1[id.sub,idx.with]
