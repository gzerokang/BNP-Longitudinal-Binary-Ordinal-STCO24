...
## square exponential covariance kernel, infinitely differentiable
var_cov_str=function(d,rho=3,sigma=1){
  sigma*exp(-d^2/(2*rho^2))
}
...
sigma.epsilon.true=0.2 ## signal to noise ratio at 5
cov.mtx.true=var_cov_str(D.mtx)
noise=rmvnorm(n,rep(0,T.all),cov.mtx.true) ## signal process is a GP
...
for (i in 1:nsim) {
  ...
  ## sample rho
  rho.cur=sample_rho(Sigma.cur,sigma.cur,a.rho,b.rho,nu.cur,length.rho=21)
